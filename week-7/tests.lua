local i = require "week-7.interpreter"
local c = require "week-7.compiler"
...
local function check(case, expect)
	...
	local top = vm.exec(code, mem, stack, {})

	print("CHECKING")
	utils.pt(stack)
	print("\nresult: " .. tostring(stack.data[1]) .. " | expected: " .. expect .. "\n")
	assert(stack.data[1] == expect, "failed: " .. case)
end
...
local cases = {
	...
	test_assign_of_x = { "x = 1; return x;", 1 },
	test_assign_x_add_y = { "x = 1; y = 2; return x + y;", 3 },
	test_block = { "{ x = 1; y = 2; }; return x + y;", 3 },
	test_empty_block = { "{};", 0 },
	test_negation = { "x = 1; return !x;", 0 },
	test_negation_multiple = { "x = 1; return !!!x", 0 },
	test_negation_multiple_eq = { "x = 1; return !!x", 1 },
	test_if = { "x = 1; if x { x = 2; return x }", 2 },
	test_else = { "x = 0; if x { x = 2; return x } else { return 2 }", 2 },
	test_elsif = { "if 0 { return 1 } elsif 1 { return 2 } else { return 4 }", 2 },
	test_and = { "1 && 20", 20 },
	test_and_short_circuit = { "0 && 20", 0 },
	test_or = { "0 || 20", 20 },
	test_or_short_circuit = { "1 || 20", 1 },
	test_array = { "a = new[10]; a[0] = 10; return a[0]", 10 },
	test_mult_array = { "a = new[10][2]; a[0][0] = 10; return a[0][0]", 10 },
}
