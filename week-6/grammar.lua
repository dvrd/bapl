...
local reserved = { "return", "if", "else", "elsif", "while", "new" }
...
local ID = (c((underscore + loc.alpha) * (loc.alnum + underscore) ^ 0) - excluded) * space
...
return p { "base",
	...
	expr  = cmp,
	term  = op_new + lhs + numeral + (token "(" * expr * token ")"),
