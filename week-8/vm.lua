...
-- Relocate top of the stack to N position below it
function Stack:drop(n)
	if self.top - n < 1 then
		print("ERROR: cant drop top of stack below first index")
		os.exit(1)
	end
	self.data[self.top - n] = self.data[self.top]
end

function Stack:update(idx)
	if #self.data < idx then
		print("ERROR: can't update out of range index")
		os.exit(1)
	end
	self.data[idx] = self.data[self.top]
end
